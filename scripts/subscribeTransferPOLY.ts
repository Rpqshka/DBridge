...
const signer = new ethers.Wallet(process.env.PRIVATE_KEY, providerETH);
...
bridgePOLYContract.on("Transfer", async (from, to, amount, date, nonce, step) => {
    if (!processedEvents.has(`${nonce}-${step}`)) {
        processedEvents.add(`${nonce}-${step}`);
        if (step.toString() == 0) {
            await bridgeETHContract.connect(signer).mint(to, amount, nonce);
